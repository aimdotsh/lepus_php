...
class Servers_redis extends Front_Controller {
    ...
    public function add(){
		...
		if(isset($_POST['submit']) && $_POST['submit']=='add')
        {
            ...
            $this->form_validation->set_rules('threshold_warning_used_memory',  'lang:alarm_threshold', 'trim|required|integer');
            $this->form_validation->set_rules('threshold_critical_used_memory',  'lang:alarm_threshold', 'trim|required|integer');
			...
			else
			{
					...
					$data = array(
                        ...
                        'threshold_warning_used_memory'=>$this->input->post('threshold_warning_used_memory'),
                        'threshold_critical_used_memory'=>$this->input->post('threshold_critical_used_memory'),
					);
              ...
              if(!empty($host) && !empty($port) &&  !empty($tags)){
                 ...
                 $data['error_code']=0;
					$data = array(
                        ...
                        'alarm_used_memory'=>$this->input->post('alarm_used_memory_'.$n),
					);
					$this->servers->insert($data);
              }
		   }

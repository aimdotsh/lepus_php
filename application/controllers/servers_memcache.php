...
class Servers_memcache extends Front_Controller {
    function __construct(){
		parent::__construct();
        $this->load->model('servers_memcache_model','servers');
    ...
    public function index(){
        ...
        $sql="select * from db_servers_memcache   where is_delete=0 $ext_where order by id asc";
        
        $result=$this->servers->get_total_record_sql($sql);
        $data["datalist"]=$result['datalist'];
        $data["datacount"]=$result['datacount'];
        
        $this->layout->view("servers_memcache/index",$data);
    }
    ...
    public function trash(){
        parent::check_privilege();
        $sql="select * from db_servers_memcache  where is_delete=1 order by id asc";
		$result=$this->servers->get_total_record_sql($sql);
        $data["datalist"]=$result['datalist'];
        $data["datacount"]=$result['datacount'];
        $this->layout->view("servers_memcache/trash",$data);
    }
    ...
    public function add(){
		...
		if(isset($_POST['submit']) && $_POST['submit']=='add')
        {
			...
			else
			{
                    ...
                    redirect(site_url('servers_memcache/index'));
            }
        }
    
        $this->layout->view("servers_memcache/add",$data);
    }
    ...
    public function edit($id){
        ...
        $this->layout->view("servers_memcache/edit",$data);
    }
     ...
     */
    function recover($id){
        parent::check_privilege('servers_memcache/trash');
        ...
        if($id){
		    ...
		    $this->servers->update($data,$id);
            redirect(site_url('servers_memcache/trash'));
        }
    }  
     ...
     */
     function batch_add(){
              ...
              if(!empty($host) && !empty($port) &&  !empty($tags)){
                 ...
                 $data['error_code']=0;
					$data = array(
                        ...
                        'alarm_connections_current'=>$this->input->post('alarm_connected_clients_'.$n),
					);
					$this->servers->insert($data);
              }
		   }
        ...
        $this->layout->view("servers_memcache/batch_add",$data);
     }
    
    
}

/* End of file servers_memcache.php */
/* Location: ./servers/controllers/servers_memcache.php */

...
class Servers_sqlserver extends Front_Controller {
    ...
    public function trash(){
        parent::check_privilege();
        $sql="select * from db_servers_sqlserver  where is_delete=1 order by id asc";
		$result=$this->servers->get_total_record_sql($sql);
        $data["datalist"]=$result['datalist'];
        $data["datacount"]=$result['datacount'];
        $this->layout->view("servers_sqlserver/trash",$data);
    }
              ...
              if(!empty($host) && !empty($port) &&  !empty($tags)){
                 ...
                 $data['error_code']=0;
					$data = array(
                        ...
                        'alarm_processes'=>$this->input->post('alarm_processes_'.$n),
                        'alarm_processes_running'=>$this->input->post('alarm_processes_running_'.$n),
						'alarm_processes_waits'=>$this->input->post('alarm_processes_waits_'.$n),
					);
					$this->servers->insert($data);
              }
		   }

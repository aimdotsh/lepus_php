...
class memcache_model extends CI_Model{
	...
	function get_status_chart_record($server_id,$time){
        $query=$this->db->query("select * from memcache_status_history  where server_id=$server_id and YmdHi=$time limit 1; ");
        if ($query->num_rows() > 0)
        {
    ...
    function check_has_record($server_id,$time){
        $query=$this->db->query("select id from memcache_status_history where server_id=$server_id and YmdHi=$time");
        if ($query->num_rows() > 0)
        {
           return true; 
        }
